...
After intake, create a dedicated generated workspace folder and place all taskiller-managed folders inside it.
...
Keep `submission/` as a top-level child of `taskiller_workspace/` so the user can see the handoff folder immediately after opening the generated workspace.
...
- Taskiller-generated working files go under `taskiller_workspace/`.
- Working files go under `taskiller_workspace/10_analysis/`, `taskiller_workspace/20_working/`, or `taskiller_workspace/20_drafts/`.
- Helper utilities created by the workflow go under `taskiller_workspace/30_tools/` when needed.
- Final outputs go under `taskiller_workspace/submission/`.
- `taskiller_workspace/submission/` must contain only project-required submission contents, not summaries, audits, helper notes, or process files.
- Do not place `.md` working files, reference templates, style exemplars, draft exports, or duplicate format variants in `taskiller_workspace/submission/` unless the assignment explicitly requires them.
